...
| **Companion**    | `opentdf-gguf-profile-design.md` (informative rationale; **superseded on wire fields** by this draft); `llama-cpp-loader-callback-handover.md` (executor binding) |
...
The profile reuses OpenTDF payload-key wrap, KAS rewrap (`POST {kas}/v1/rewrap`), AES-256-GCM, GMAC segment tags, the HS256 root-signature **formula**, and policy binding **without change to those algorithms**. **When** the root signature is verified on the inference path is a profile deviation (§4, §10.4): MUST before any weight byte (`offset >= headerBytes`); MAY skip only for header-only / `vocab_only` loads. The profile does **not** replace OpenTDF. Generic tooling that only decrypts `payload.url` will decrypt the header member, not reconstruct a vanilla `.gguf`. Reconstructing a vanilla `.gguf` with `otdfctl decrypt` is **not** a v1 requirement.
...
This is a Community Draft (`draft-00`). The profile identifier `gguf-tdf/1` is the on-the-wire version. A future `gguf-tdf/2` would be a new profile string; readers of `gguf-tdf/1` MUST refuse unknown profile strings (fail closed).
...
| **KAS** | Key Access Service. Rewrap protocol is unchanged: `POST {kas}/v1/rewrap`. |
...
| **Fail closed** | On KAS, policy, profile, size, or tag errors: abort; emit no plaintext weights; do not fall back to a sibling `.gguf`. |
...
4. Variable segment sizes aligned to GGUF layout (header / packed small tensors / split large tensors).
...
JSON Schema: [`schemas/gguf-tdf/draft-00/gguf-index.schema.json`](../schemas/gguf-tdf/draft-00/gguf-index.schema.json).
...
On any invariant failure the reader MUST fail with `GGUFTDF_SIZE_MISMATCH` or `GGUFTDF_BAD_INDEX` and MUST NOT decrypt weights.
...
### 10.6 KAS rewrap (unchanged)

Writers and readers MUST use OpenTDF KAS rewrap. Readers:

1. Authenticate to KAS (OAuth / CWT / deployment-specific; out of scope except that a token is REQUIRED).
2. `POST {keyAccess.url}/v1/rewrap` with wrapped key, policy, policy binding, and the caller's ephemeral public key.
3. Unwrap the rewrapped key to the 32-byte payload key.

This profile MUST NOT change the KAS URL path, request schema, or wrap algorithms. Failures (401/403 authn, 403 authz/policy deny, binding failure, network) are `GGUFTDF_KAS_DENIED`. Readers MUST NOT fall back to a sibling plaintext `.gguf`.
...
The while-condition is **`>=`**, not `>`. A remainder of exactly `maxSegment` MUST emit a member (Appendix A: after `s/1` the rest of the 256-byte tensor is exactly 128 bytes). There is no `GGUFTDF_INTERNAL`. If an open pack already holds at least `maxSegment` bytes of prior tensors/padding (`remaining_off - pack_start >= maxSegment`), close it **before** filling from the new tensor.

**Note on the design-doc pseudocode:** when an open pack already contains prior tensors/padding and is not yet a full cap, the emitted range is a **full** `maxSegment` window `[pack_start, pack_start + maxSegment)`, not a window of length `maxSegment - (remaining_off - pack_start)`. The latter quantity is the number of **new** tensor bytes added to fill the cap.
...
for each tensor t in GGUF order:
  ...
  # Close an open pack that is already a full cap of prior data/padding.
  # Example: 50 B tensor + 78 B padding with maxSegment=128; next tensor
  # starts at pack_start+128. Never fail INTERNAL on add == 0.
  while remaining_off - pack_start >= maxSegment:
    emit_range := [pack_start, pack_start + maxSegment)
    kind := "pack" if emit_range intersects two or more tensors else "tensor"
            # padding-only window is kind=pack
    if emit_range intersects no tensor: kind := "pack"
    emit kind, emit_range, entry="s/{next_id}", id=next_id
    next_id    := next_id + 1
  ...
  while remaining_end - pack_start >= maxSegment and remaining_end > remaining_off:
    if pack_start == remaining_off:
      # new segment starts on tensor data (or an aligned split point).
      # The loop condition guarantees remaining_end - pack_start >= maxSegment,
      # so the window is always a full maxSegment (a multiple of ALIGN).
      emit_range := [pack_start, pack_start + maxSegment)
    else:
      # open pack holds [pack_start, remaining_off); that length is < maxSegment
      # because of the previous while. Fill to maxSegment.
      emit_range := [pack_start, pack_start + maxSegment)
...
if virtualSize > pack_start:
  emit_range := [pack_start, virtualSize)
  kind := "pack" if emit_range intersects two or more tensors
          else "tensor" if it intersects one
          else "pack"   # trailing padding only
  emit kind, emit_range, entry="s/{next_id}", id=next_id
...
2. Tensor A: `remaining_end - pack_start = 256 >= 128`, `pack_start == remaining_off`. `take=128`. Emit `s/1` `[64, 192)` `kind=tensor`.
3. `pack_start=192`. Remainder of A is 128. `128 >= 128`. Emit `s/2` `[192, 320)` `kind=tensor`.
4. Tensor B: `352 - 320 = 32 >= 128` is false. Leave in open pack. `cursor=352`.
5. Final: emit `s/3` `[320, 352)` `kind=tensor` (one tensor; not a pack).
...
This is T15 / Appendix A / `example-manifest.json`. Writers that implement `>` instead of `>=` will not emit `s/2` and are non-conforming.

The last segment MUST extend to `virtualSize` so trailing source bytes are preserved. That last segment MAY have `plain < maxSegment` and is not required to be a multiple of `ALIGN`. The header segment MAY have `plain > maxSegment`. Every other segment MUST have `plain <= maxSegment`. Intermediate split points of a large tensor MUST lie on `ALIGN` boundaries.

### 11.4 Consequences

- A 16 MiB tensor with `maxSegment = 4 MiB` becomes four `kind=tensor` members.
- Small tensors (LayerNorm, few KiB) accumulate in an open pack until adding the next tensor would make `remaining_end - pack_start >= maxSegment`; then the pack is closed. A small tensor **may** still be split if it straddles a `maxSegment` boundary.
...
flowchart TD
  ...
  C --> P{"prior pack already >= maxSegment?"}
  P -->|yes| Q["Emit full maxSegment window of prior data"]
  Q --> P
  P -->|no| D{"open pack + this tensor >= maxSegment?"}
  D -->|yes| E["Emit full maxSegment window<br/>kind tensor or pack"]
  E --> D
  D -->|no| F["Keep tensor tail in open pack"]
  F --> C
  C --> G["Emit final segment through virtualSize"]
```
...
Owner (informative): `arkavo-tdf` + CLI `arkavo model protect`. Not MCP `tdf_encrypt` / `TdfJsonRpc`.
...
7. MUST NOT delete the source `.gguf` unless the caller passes an explicit flag. Default: leave plaintext where it was; the protected artifact is additive.
8. MUST NOT write a **new** plaintext sidecar `.gguf` next to the `.gguf.tdf` after wrap.

Wrap is additive, so `model.gguf` and `model.gguf.tdf` MAY both exist. Discovery and load MUST follow §13.1: a TDF-capable reader MUST use the `.gguf.tdf`, not the sibling plaintext. HuggingFace-cache deployments SHOULD store `.gguf.tdf` as the artifact and SHOULD NOT also write `.gguf` after download-and-protect.

---

## 13. Unwrap-on-Load Procedure

Owner (informative): Edge `arkavo-llm` / `arkavo-tdf` helper. llama.cpp sees only the callback.
...
If both `model.gguf` and `model.gguf.tdf` exist (same basename, wrap-additive layout):

- TDF-capable readers MUST load `model.gguf.tdf` (or fail closed). They MUST NOT open the sibling `.gguf` as the model.
- Readers without TDF/KAS features MUST refuse `.gguf.tdf`. They SHOULD NOT silently load the sibling `.gguf` when a `.gguf.tdf` is present, unless the operator sets an explicit flag that acknowledges plaintext load of a protected model.
- Path-agnostic directory scans (`model_discovery`, HuggingFace cache) MUST apply the same preference. T12 and T14 test this.

This is distinct from `GGUFTDF_KAS_DENIED` (no fallback **after** a rewrap failure). Discovery MUST NOT bypass KAS by picking the plaintext file first.
...
| `GGUFTDF_KAS_DENIED` | KAS unreachable, authn/authz fail, policy deny, binding fail | Error; no sibling `.gguf` |
...
| `GGUFTDF_SIBLING_REFUSED` | TDF-capable reader asked to prefer sibling `.gguf` over `.gguf.tdf` | Error / refuse; MUST use `.gguf.tdf` |
| `GGUFTDF_PAYLOAD_FORBIDDEN` | Archive contains `0.payload` **and** profile members | Writers MUST NOT produce this. Readers SHOULD ignore `0.payload` if `gguf` is valid; MUST NOT decrypt `0.payload` as GGUF. |
...
- **C-R3.** Fail closed on KAS deny; no sibling `.gguf` fallback. If both `.gguf` and `.gguf.tdf` exist, use `.gguf.tdf` (§13.1).
...
| **T12** | Discovery: directory containing both `model.gguf` and `model.gguf.tdf` — TDF-capable reader loads `.gguf.tdf` (T14 covers the preference MUST). |
...
| **T14** | Both `model.gguf` and `model.gguf.tdf` present: TDF-capable reader MUST use `.gguf.tdf`; MUST NOT load the sibling `.gguf`. |
...
Published expected plan (T15). Fixture files: `schemas/gguf-tdf/draft-00/example-manifest.json` and `appendix-a-packing-plan.json`.
...
| Arkavo without TDF/KAS features | Refuse `.gguf.tdf`. SHOULD NOT silently load a sibling `.gguf` when `.gguf.tdf` is present unless an explicit operator flag says so |
...
Weights and tokenizer **bytes** are in the encrypted header segment. If tokenizer secrecy matters, it is already covered. Tensor **names** are not secret in this profile. Encrypting the index is deferred to a future profile (`gguf-tdf/2`), not an open v1 decision.
...
Readers MUST cap decompressed size by `encryptedSegmentSize` from the manifest (Stored: no expansion). Readers MUST only decrypt members named in `gguf.segments[].entry`. Extra zip members are ignored. A `0.payload` member MUST NOT be treated as GGUF bytes.
...
|---|---|
| `opentdf-rs` | Variable-length segments, multi-entry zip, `decrypt_entry`, optional `gguf` on the manifest. Keep rustls-only. KAS feature remains `kas` / `kas-client`. |
...
| `arkavo-llm` | Path `.gguf.tdf` → KAS + callback load instead of `LlamaModel::from_file`. |
| `arkavo-router` `model_discovery` | If both `.gguf` and `.gguf.tdf` exist, TDF-capable discovery MUST select `.gguf.tdf` (§13.1). |
| CLI | `arkavo model protect`. |
...
| Crypto | One payload key, one KAS policy, AES-256-GCM, GMAC, HS256 root sig | Same as zip TDF; KAS wire protocol unchanged |
...
| Fail closed | KAS/policy/tag/profile errors abort; no sibling `.gguf` | A deny that falls back to plaintext is not protection |
...
|---|---|---|---|---|
| **PR1** | `spec: add gguf-tdf/1 draft-00` | `gguf-tdf/draft-arkavo-gguf-tdf-00.md`, `schemas/gguf-tdf/draft-00/*` (including Appendix A packing plan), `README.md` | — | This document, schema, and packing vector. Mergeable alone. |
...
| **PR4** | `arkavo-tdf: gguf-tdf/1 packer + read_at` | packer, segment plan, userdata `read_at`, mock-KAS tests **T1–T11, T13, T15–T18** | PR2a | Normative packing (`>=`) + decrypt-on-load. No llama.cpp link. T11 (fallback `manifest.json`) and T18 (endian/magic) are here. |
| **PR5** | `cli: arkavo model protect` | Arkavo CLI | PR4 | Wrap path; additive default; explicit delete-source flag only. |
| **PR6** | `arkavo-llama-cpp: LlamaModel::from_callback` | `arkavo-llama-cpp/src/callback.rs`, `lib.rs` | — | Cookie `FILE*` (`funopen` / `fopencookie`) → `llama_model_load_from_file_ptr`. `setvbuf` ≥ 1 MiB. Still no TDF. Tests: cookie seek/read, zero size, non-GGUF reject, optional `ARKAVO_TEST_MODEL` parity with `from_file`. |
| **PR7** | `arkavo-llm: load .gguf.tdf via KAS + callback` | `arkavo-llm` | PR4, PR6 | Path detect; fail closed without KAS; sibling `.gguf` MUST NOT win. I1. |
| **PR8** | `arkavo-router: discover .gguf.tdf` | `model_discovery` | PR7 optional | If both exist, TDF-capable MUST use `.gguf.tdf`. Tests **T12, T14**. |
